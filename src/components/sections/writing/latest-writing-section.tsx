...
export function LatestWritingSection({
  ...
  return (
    <section id="writing" className="atlas-section" aria-labelledby="writing-title">
        ...
        <div className="atlas-section-heading">
          <div className="flex flex-wrap items-end justify-between gap-4">
            ...
            {result.status === "ready" ? (
              <a
                href={blog.homeUrl}
                className={`${buttonVariants({ variant: "outline" })} atlas-button group`}
              >
                {copy.visitBlog}{" "}
                <LuArrowUpRight
                  aria-hidden="true"
        ...
        {visiblePosts.length > 0 ? (
          <div className="writing-grid mt-8 grid gap-px bg-border" data-count={visiblePosts.length}>
            {visiblePosts.map((post) => (
              <article
                key={post.slug}
                className="atlas-writing-card group flex min-h-68 flex-col bg-background p-[clamp(1.25rem,3vw,2rem)] transition-colors duration-[var(--motion-feedback)] ease-[ease]"
              >
                ...
                <h3 className="mt-[0.65rem] max-w-[23ch] text-[clamp(1.35rem,2vw,1.8rem)] leading-[1.05] font-[750] tracking-[-0.035em]">
                  <a
                    ...
                    className="flex min-h-11 items-center no-underline transition-colors duration-[var(--motion-feedback)] ease-[ease] [overflow-wrap:anywhere] hover:text-[var(--accent-strong)] focus-visible:text-[var(--accent-strong)]"
                  >
                    {post.title}
                  </a>
                </h3>
                ...
                <div className="mt-auto flex items-end justify-between gap-4 pt-6 font-mono text-[0.63rem] text-muted-foreground">
                  ...
                  <LuArrowUpRight
                    aria-hidden="true"
                    className="size-5 text-[var(--accent-strong)] transition-transform duration-[var(--motion-feedback)] ease-[ease] group-hover:translate-x-1 group-hover:-translate-y-1 group-focus-within:translate-x-1 group-focus-within:-translate-y-1 motion-reduce:transform-none"
                  />
                </div>
              </article>
            ))}
            {visiblePosts.length === 1 ? (
              <aside
                className="atlas-writing-aside flex min-h-68 flex-col justify-between bg-[var(--atlas-route-blue)] p-[clamp(1.25rem,3vw,2rem)] text-[var(--atlas-survey-paper)]"
                aria-labelledby="writing-aside-title"
              >
                <p className="atlas-kicker">{copy.asideEyebrow}</p>
                <h3
